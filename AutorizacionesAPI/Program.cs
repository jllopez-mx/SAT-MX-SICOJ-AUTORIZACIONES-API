#region Usings
using AutorizacionesAPI.ServiceRegistration;
...
#region Inyeccion de validadores
...
{
    option.SwaggerDoc("v1", new OpenApiInfo { Title = "Autorizaciones API", Version = "v1" });
...
try
{
    ...
    string serverHttp = builder.Configuration.GetValue<string>("LaunchSettings:HostHttp")!;
    string serverHttps = builder.Configuration.GetValue<string>("LaunchSettings:HostHttps")!;
    string swagger = builder.Configuration.GetValue<string>("LaunchSettings:Swagger")!;
    logger.LogInformation("-------------------------------------------------------------------");
    logger.LogInformation($"| SERVER HTTP ON: {serverHttp}");
    logger.LogInformation($"| SWAGGER HTTP ON: {serverHttp}{swagger}");
    logger.LogInformation("-------------------------------------------------------------------");
    logger.LogInformation($"| SERVER HTTPS ON: {serverHttps}");
    logger.LogInformation($"| SWAGGER HTTPS ON: {serverHttps}{swagger}");
    logger.LogInformation("-------------------------------------------------------------------");
...
{
    logger.LogCritical(_e, _e.ManageException());
}
#endregion